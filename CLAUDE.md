...
| everywhere                                                | `bun run ci` (`ci:fast` skips the suites)    | `scripts/ci.mjs`, plain node                          | Chrome, for css · network, for `scaffold`           |
...
**`bun run ci` is the whole of CI and it is not a GitHub feature.** One node script runs eight phases — hygiene, structure, snapshots, coverage, scaffold, typecheck, deploy, tests — and `.github/workflows/ci.yml` does nothing but call it, so it runs identically on a laptop. It fails a package with no `test` script, a source file `.gitignore` hides, a *raised* typecheck baseline, a committed snapshot that no longer matches its source, and an **architecture rule broken in this repo's own apps** — `structure` runs `packages/cli/core/checks.js`, the same engine `fli check` gives a client app, because two implementations of one rule is how a framework ends up breaking rules it publishes. **`snapshots` carries no list of what exists**: every committed `*.snapshot.*` names the command that wrote it in its own header (`generated by: litestone ddl --schema schema.lite`), and the phase reruns that command with `--check` from the file's own directory — so a new KIND of snapshot costs a generator, not a CI edit. The command comes out of a file in the repo, so it is restricted to a known binary and a shell-free argv. **Discovery alone fails open**, so the expectation comes from the BASE REF: a snapshot tracked where the branch left main and absent now is a failure, because a generated artefact that stops existing takes its check with it in green. A deliberate removal is a named entry under `removedSnapshots` in `scripts/ci-allowances.json`. **`scaffold` is the phase no suite can replace**: it runs `bun pm pack` over every publishable package, scaffolds an app against the tarballs, installs it, builds it, **scaffolds a model into it and builds again** (`scripts/scaffold-build.mjs`). An app in this repo resolves sierra to `packages/sierra/` and never to a node_modules path, so the code that only runs for an *installed* app runs nowhere else here — which is how `FJS-251` broke every npm install past 836 green sierra tests. It grades each package's `files:` field for free, and **its per-run `--cache-dir` is load-bearing**: bun caches a tarball by name *and* version, so a shared cache serves the previous run's bytes and the phase passes against a broken tree. **`deploy` is the only phase that tests the PUBLISHED framework** — `fli new --source npm` → `make:deploy` → `deploy:local`, an image built and a health answer — because a Docker build cannot see a `file:` tarball outside its context (`FJS-241`). It needs a daemon that can read the build context: a shell with a private `/tmp` gets `unable to prepare context` about a directory that is plainly there, and `FJS_CI_WORKDIR` is the way out. **Without a daemon it reports a named note and passes** — requiring Docker for `bun run ci` on a laptop is too much — and `FJS_CI_REQUIRE_DOCKER=1` turns that skip into a failure, which the workflow sets, because a check that quietly stops running is worse than one nobody wrote. A suite known to fail is named so a NEW failure is loud. Every allowance is a named entry with a reason in `scripts/ci-allowances.json`. `bun run hooks:install` points `core.hooksPath` at `scripts/hooks`, whose `pre-push` runs the fast tier (~40s); the suites (~3 min) belong in the workflow, because a pre-push costing minutes teaches everyone `--no-verify`. `--only <pkg>` narrows typecheck and tests.
...
- **`@guarded` is not a level** — a system-context lock taking only `(all)`; `@guarded(5)` does not parse. Per-role column access is field-level `@allow('read', auth().role == 'admin')`. It is also **read-side only**: a guarded column is stripped from the ANSWER while the write lands, which reads exactly like a refusal (`FJS-248`). The write tool is `@allow('write', …)`.
...
- **What the API answers is not readable in a service file, so it is committed too.** `junction surface --app <module> [--services <dir>]` writes `surface.snapshot.md` at the app root — methods after the policy, actions as `collectActions` resolved them at construction, the hook chain in RUN order with the derived hooks (`gateAuth`, `autoValidate`) leading it, every mounted path with `apiPrefix` applied, plugins in configure order. Read off a BUILT app, so the module must expose one **without listening** (guard an entry's `app.start()` with `import.meta.main`) and autoloaded services need `--services`. The `snapshots` CI phase fails a stale one.
