...
| **Open Food Network** | `db/schema.rb` · 64 KB | `rails` | commerce with a supply chain and many enterprises over one catalog. The only Rails commerce schema that is actually fetchable — Spree, Solidus and Redmine ship **no** `db/schema.rb`, which the probe is how we know |
