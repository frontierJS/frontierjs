...
| **`lexicon`** | UI | i18n. Flagged tier-1 elsewhere for the right reason: every string written before this exists is a string to find again later. Its design question — do labels derive from the schema? — must be answered **before** `foundry` ships, since a generated form generates labels. | `ecosystem-gaps.md` tier-1 item 4 |
