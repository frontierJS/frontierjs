...
4. `core/crud-templates.js` becomes a consumer, the way it consumed `columns()`.
5. **Three adopters, chosen to disagree**: `example` invoices takes the ranking pinned and the URL as its state, `basecamp` deployments takes it unaided and local, and one embedded list takes a `where`. Two callers measure nothing, and the mode that nineteen of twenty real lists want has to have one.
