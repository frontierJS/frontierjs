...
5. **Three adopters, chosen to disagree — two landed, and the third is a question.**
   ...
   - **`basecamp` deployments, `state: 'local'`, ranking unaided — not adopted, because the list's rows are COMPOSED.** `deployments.find` includes the app and the environment, and a live push carries the row alone; the store's `upsert` REPLACES a row, so the first step transition of a running release would blank both relation cells. The page merges pushes by hand today for exactly that reason. `record(id, { composed: true })` is the same shape for one row and answers it by re-reading (`FJS-D161`); a list has two answers and they cost differently — **re-run the load on a push** (correct, and a query per step of every concurrent release, which the page's own comment calls a poll), or **merge the push over the held row** (free, and stale the day a pushed row changes the key an include was read through). Unruled.
