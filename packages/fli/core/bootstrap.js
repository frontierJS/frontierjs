...
// Global env (~/.config/fli/.env) — loaded first so project .env can override
loadEnv(resolve(homedir(), '.config', 'fli', '.env'))
// Project env — overrides globals
loadEnv(resolve(global.projectRoot, '.env'))
...
function printHelp(meta, filePath) {
  ...
  // Flags — skip internal ones
  const flags = Object.entries(meta.flags || {}).filter(([k]) => k !== 'test' && k !== '_spec')
  if (flags.length) {
    logger('\n  Flags:', 'info')
    for (const [name, def] of flags) {
...
export async function run(process) {
  try {
    ...
    const { help: _help, h: _h, ...cmdFlag } = flag
