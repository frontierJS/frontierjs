...
`@frontierjs/toolbelt` v0.1.4 — pure-function kits, one per subpath, zero
...
| | |
|---|---|
| Tests | **407 passing, 0 failing**, 16 spec files (`bun run test`) — verified |
| Under node | matching pass count (`node test/run.js`) — verified. The two runtimes now answer identically, which is the point of the section below |
| Typecheck | **clean, no baseline** (`bun run typecheck`) — verified |
| Substrate purity | no dependency and no ambient capability — verified, from `bun run ci`'s hygiene phase |
| Published | `0.1.4` on npm, the version the tree carries (`npm view @frontierjs/toolbelt version`) — verified. Every declared entry point is in the tarball (`exports.snapshot.md`) |
