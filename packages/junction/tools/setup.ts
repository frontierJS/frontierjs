...
import {
  c, paint, ok, warn, fail, note,
  dim as dimText, header as headerText, sep as sepLine,
  createPrompter,
} from './ui.ts'
import {
  type GenInput,
...
// setup.ts spacing conventions on top of the shared ui helpers:
// dim/header carry their own indentation, and the separator is 56 wide.
const dim    = (t: string) => `  ${dimText(t)}`
const header = (t: string) => `\n  ${headerText(t)}`
const sep    = () => sepLine(56)
...
const { rl, ask, confirm } = createPrompter()
...
// Content comes from the canonical generators in tools/generators.ts —
// these wrappers just decide where to write and whether to overwrite.

function writeOut(rel: string, content: string): void {
...
function writeConfigFile(name: string, port: number): void {
  writeOut('config/default.ts', genDefaultConfig({ projectName: name, port }))
}

function writeProductionConfig(origins: string[]): void {
  if (exists('config/production.ts')) return  // don't overwrite
  writeOut('config/production.ts', genProductionConfig({ corsOrigins: origins }))
}

function writeAppEntryPoint(): void {
  writeOut('app.ts', genAppEntryPoint({}))
}

function writeStarterService(name: string): void {
  writeOut(`services/${name}.service.ts`, genService(name))
}
...
async function scaffoldNewProject(): Promise<void> {
  ...
  const o: GenInput = { projectName: n, port: p }

  writeOut('package.json', genPackageJson(o))
  writeOut('.env',         genDotEnv(o))
  writeOut('.gitignore',   genGitignore())
  writeConfigFile(n, p)
