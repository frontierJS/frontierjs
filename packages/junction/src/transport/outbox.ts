...
const outboxes = new WeakMap<object, Outbox>()

/** Frames currently held for this socket. 0 when it is keeping up. */
export function queuedFrames(ws: OutboxSocket): number {
  return outboxes.get(ws as object)?.frames.length ?? 0
}
 ...
 */
export function wsSend(ws: OutboxSocket, payload: string): 'sent' | 'queued' | 'closed' {
  if (ws.readyState !== 1) return 'closed'

  const held = outboxes.get(ws as object)

  // Anything held means everything queues — see the ordering note above.
  if (held && held.frames.length > 0) return enqueue(ws, held, payload)
  ...
  return enqueue(ws, held ?? newOutbox(ws), payload)
}
 ...
 */
export function flushOutbox(ws: OutboxSocket): void {
  const held = outboxes.get(ws as object)
  if (!held || held.frames.length === 0) return

  if (ws.readyState !== 1) {
    outboxes.delete(ws as object)
    return
  }
  ...
  while (held.frames.length > 0) {
    ...
    let result: number
    try { result = ws.send(frame) } catch { outboxes.delete(ws as object); return }
    if (result === 0) return
    held.frames.shift()
...
export function dropOutbox(ws: OutboxSocket): void {
  outboxes.delete(ws as object)
}

function newOutbox(ws: OutboxSocket): Outbox {
  const box: Outbox = { frames: [], bytes: 0 }
  outboxes.set(ws as object, box)
  return box
}

function enqueue(ws: OutboxSocket, held: Outbox, payload: string): 'queued' | 'closed' {
