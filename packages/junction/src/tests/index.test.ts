...
import { matchPathDirect } from '../transport/router.ts'
import type { RouteSegment } from '../transport/types.ts'
...
import { parseBody, parseQuery, parseCookies, extractIP } from '../transport/body.ts'
...
import { runPipeline, resolvePipelines, mergeHookMaps } from '../core/hooks.ts'
...
function makeCtx(overrides: Partial<ServiceContext> = {}): ServiceContext {
  return {
    ...
    query:     {},
    data:      null,
    params:    { headers: {}, ip: '127.0.0.1', user: null },
    app:       {} as import('../core/app.ts').App,
    result:    null,
    error:     null,
    $raw:      null,
    ...overrides
...
import { createTestApp as makeApp2, request as req2 } from '../testing/index.ts'
...
describe('protect() dot-path support', () => {
  ...
  it('strips a top-level field (existing behaviour)', async () => {
    ...
    const res = await _request(app).get('/api/users')
  ...
  it('handles missing intermediate keys gracefully', async () => {
    ...
    const res = await _request(app).get('/api/items')
...
import { defineEnv, generateEnvExample } from '../core/env.ts'
...
import { parseQuery as parseLSQuery, parseWhere, deriveModelName } from '../core/litestone.ts'
...
describe('Bridge', () => {
  ...
  it('toResponse returns error response when ctx.error is set', () => {
    const { BadRequest } = require('../core/errors.ts')
...
describe('Service custom methods', () => {
  ...
  it('mergeHookMaps preserves custom method keys', () => {
    ...
    const { mergeHookMaps } = require('../core/hooks.ts')
    const merged = mergeHookMaps(a, b)
  ...
  it('resolvePipelines includes custom method pipelines', () => {
    const { resolvePipelines } = require('../core/hooks.ts')
...
import { createChannelManager, Channel, publish } from '../transport/channels.ts'
...
describe('ChannelManager', () => {
  ...
  it('publish sends to target channels', async () => {
    ...
    const fakeCtx = { service: 'deployments', method: 'create', params: { user: null } } as any
...
import { createDatabase, createInMemoryDatabase } from '../storage/database/index.ts'
...
describe('createDatabase', () => {
  ...
  it('createDatabase integrates with createApp', async () => {
    const { createApp }    = await import('../core/app.ts')
    const { defaultConfig } = await import('../config/index.ts')
...
import { createTestApp, createStubAuth, request, testCtx } from '../testing/index.ts'
import { createService }                                     from '../core/service.ts'
import { callService }                                       from '../core/service.ts'
...
describe('testCtx', () => {
  ...
  it('creates a service context for direct testing', () => {
    ...
    expect(ctx.params.user?.userId).toBe('u1')
    expect(ctx.params.workspaceId).toBe('ws-1')
  })
})
  ...
  it('sets Authorization header via .auth()', async () => {
    ...
    const app = await createTestApp({
      services: [(_app) => createService({
        name: 'echo',
        find: async (ctx) => {
          receivedToken = ctx.params.headers['authorization']
          return []
        },
      })]
    })

    await request(app).get('/api/echo').auth('my-token')
...
import { generateOpenAPI, openapi } from '../plugins/openapi/index.ts'
import { createSchema, v }          from '../core/schema.ts'
...
describe('generateOpenAPI', () => {
  ...
  it('includes collection and resource paths for each service', async () => {
    ...
    expect(spec.paths['/api/posts']).toHaveProperty('get')
    expect(spec.paths['/api/posts']).toHaveProperty('post')
    expect(spec.paths['/api/posts/{id}']).toHaveProperty('get')
    expect(spec.paths['/api/posts/{id}']).toHaveProperty('patch')
    expect(spec.paths['/api/posts/{id}']).toHaveProperty('delete')
  })
  ...
  it('includes action routes', async () => {
    ...
    expect(spec.paths['/api/posts/{id}/publish']).toBeDefined()
    expect(spec.paths['/api/posts/{id}/unpublish']).toBeDefined()
  })
  ...
  it('includes schema details when schemas are registered', async () => {
    ...
    const createOp = spec.paths['/api/posts'].post
...
describe('ctx.statusCode', () => {
  ...
  it('find/get/patch/remove return 200', async () => {
    ...
    expect((await request(app).patch('/api/items/1').send({})).status).toBe(200)
  })
  ...
  it('service can override status to 202', async () => {
    ...
    const res = await request(app).post('/api/jobs').send({ type: 'export' })
    expect(res.status).toBe(202)
  })
  ...
  it('null result returns 204', async () => {
    ...
    const res = await request(app).post('/api/pings').send({})
    expect(res.status).toBe(204)
  })
})
...
describe('ctx.dispatch', () => {
  ...
  it('publish() uses ctx.result when dispatch not set', async () => {
    ...
    const app = await createTestApp({
      services: [() => createService({
        ...
        hooks: {
          after: {
            create: [async (ctx) => { ctx.params.__channels = manager },
    ...
    await request(app).post('/api/posts').send({ title: 'Hello' })
  ...
  it('publish() suppresses broadcast when dispatch === false', async () => {
    ...
    const app = await createTestApp({
      services: [() => createService({
        ...
        hooks: {
          after: {
            create: [
              async (ctx) => { ctx.params.__channels = manager },
              publish(() => manager.channel('all')),
            ]
          }
        }
    ...
    await request(app).post('/api/internal').send({})
...
describe('app.service()', () => {
  ...
  it('find() returns the service result', async () => {
    const app = await createTestApp({
      services: [() => createService({
        name: 'items',
  ...
  it('params flow through to ctx.params when provided', async () => {
    const app = await createTestApp({
      services: [() => createService({
        name: 'items',
        find: async (ctx) => ({ workspace: (ctx.params as Record<string, unknown>).workspaceId }),
      })],
    })
    const result = await app.service('items').find({ workspaceId: 'ws-99' } as never) as Record<string, unknown>
    expect(result.workspace).toBe('ws-99')
  })

  it('passing params threads user into ctx.params.user', async () => {
    const app = await createTestApp({
      services: [() => createService({
        name: 'items',
        find: async (ctx) => ({ callerUser: ctx.params.user?.userId ?? null }),
    ...
    const result = await app.service('items').find({ user: fakeUser }) as Record<string, unknown>
...
describe('Auto-events', () => {
  ...
  it('create fires service:created', async () => {
    ...
    await request(app).post('/api/notes').send({ title: 'Hello' })
    await new Promise(r => setTimeout(r, 10))
  ...
  it('patch fires service:patched', async () => {
    ...
    await request(app).patch('/api/notes/1').send({ title: 'Updated' })
    await new Promise(r => setTimeout(r, 10))
  ...
  it('find and get do NOT fire auto-events', async () => {
    ...
    await request(app).get('/api/notes/1')
    await new Promise(r => setTimeout(r, 10))
    expect(eventCount).toBe(0)
  })
...
describe('Bidirectional WS service calls', () => {
  ...
  async function makeWsApp() {
    const app = await createTestApp({
      ...
      services: [() => createService({
        ...
        create: async (ctx) => ({ id: '2', text: (ctx.data as Record<string, unknown>)?.text, author: ctx.params.user?.userId }),
      })]
    })
  ...
  it('service_call dispatches to service and returns service_result', async () => {
    const { createChannelManager: mgr } = await import('../transport/channels.ts')
    const manager = mgr()
    ...
    const { callService: _call } = await import('../core/service.ts')
    const { bridge: _bridge }    = await import('../transport/bridge.ts')

    const svc    = app.services.get('pings')!
    const svcCtx = _bridge.internal('pings', 'find', null, {
      user:       conn.user as any,
      __channels: manager,
    })
    svcCtx.transport = 'websocket'
    await _call(svc, svcCtx, undefined, app.events)

    expect(Array.isArray(svcCtx.result)).toBe(true)
    expect((svcCtx.result as unknown[])[0]).toMatchObject({ pong: true })
    manager.destroy()
  })
...
import { createService }                from '../core/service.ts'
import { callService }                  from '../core/service.ts'
import { authenticate, requireRole, circuitBreaker } from '../core/hooks.ts'
import { createTestApp, request, testCtx } from '../testing/index.ts'
import { correlationId }                from '../transport/middleware.ts'
import { healthPlugin }                 from '../transport/health.ts'
import { BadRequest, Unauthorized, Forbidden, Unavailable } from '../core/errors.ts'
import { bridge }                       from '../transport/bridge.ts'
...
describe('_compiledPipelines', () => {
  ...
  it('are null before start and populated after app.hooks() + start()', async () => {
    ...
    const { resolvePipelines, mergeHookMaps } = await import('../core/hooks.ts')
    const svc = app.services.get('pings')!
...
import { rateLimit } from '../core/hooks.ts'
import { TooManyRequests } from '../core/errors.ts'
...
describe('rateLimit hook', () => {
  ...
  it('keys on IP for anonymous requests', async () => {
    ...
    ;(ip1.params as Record<string, unknown>).ip = '1.2.3.4'
    const ip2 = testCtx('items', 'find')
    ;(ip2.params as Record<string, unknown>).ip = '5.6.7.8'
...
describe('csrf middleware', () => {
  ...
  it('allows multiple listed origins', async () => {
    ...
    const r1 = await request(app).post('/api/notes').set('origin', 'https://app.com').send({})
    expect(r1.status).toBe(201)

    const r2 = await request(app).post('/api/notes').set('origin', 'https://admin.app.com').send({})
  ...
  it('blocks mutating request with no origin by default', async () => {
    ...
    const res = await request(app).post('/api/notes').send({ title: 'No origin' })
    expect(res.status).toBe(403)
  })
  ...
  it('allows missing origin when allowMissingOrigin is true', async () => {
    ...
    const res = await request(app).post('/api/notes').send({ title: 'Server call' })
    expect(res.status).toBe(201)
  })
...
describe('apiPrefix config', () => {
  ...
  it('defaults to /api — existing routes still work', async () => {
    const app = await createTestApp({
...
import { createLogger, consoleWriter, fileWriter, noopLogger } from '../core/logger.ts'

describe('createLogger', () => {

  it('logs at all levels without throwing', () => {
    const logger = createLogger({ level: 'debug', writers: [noopLogger as unknown as import('../core/logger.ts').LogWriter] })
  ...
  it('child logger inherits namespace and defaults', () => {
    const entries: import('../core/logger.ts').LogEntry[] = []
    const writer = (e: import('../core/logger.ts').LogEntry) => entries.push(e)
...
import { autoloadServices } from '../core/loader.ts'
...
describe('autoloadServices', () => {
  ...
  it('loads service files from a directory', async () => {
    ...
    await Bun.write(`${dir}/users.service.ts`, `
      import { createService } from '../../framework/core/service.ts'
      export function createUsersService() {
        return createService({ name: 'users', find: async () => [] })
      }
    `)

    const { ServiceRegistry } = await import('../core/service.ts')
  ...
  it('skips duplicate services', async () => {
    ...
    const { ServiceRegistry, createService } = await import('../core/service.ts')
    const registry = new ServiceRegistry()
...
import { createLitestoneService } from '../core/litestone.ts'
...
// ─── createLitestoneService — service wiring ──────────────────────────────
// Regression test for the bug where createLitestoneService returned a plain
// ServiceDefinition object instead of calling createService(), meaning
// _hookMap and _pipelines were undefined and app.start() crashed in
// _recompileAll → mergeHookMaps.

describe('createLitestoneService', () => {
  ...
  it('registered service responds to GET /api/widgets', async () => {
    const app = await createTestApp({
      services: [() => createLitestoneService({
        name:  'widgets',
  ...
  it('hooks declared on createLitestoneService are applied', async () => {
    let hookRan = false
    const app = await createTestApp({
      services: [() => createLitestoneService({
        name: 'widgets',
...
describe('Cache-Control headers', () => {
  ...
  it('authenticated GET returns private, no-cache', async () => {
    const app = await createTestApp({
      ...
      auth: createStubAuth([{ userId: 'u1', role: 'user' }]),
    })
    const res = await request(app)
      .get('/api/items')
  ...
  it('PATCH returns no-store', async () => {
    ...
    const res = await request(app).patch('/api/items/1').send({ name: 'y' })
    expect(res.status).toBe(200)
    expect(res.headers['cache-control']).toBe('no-store')
  })
})
