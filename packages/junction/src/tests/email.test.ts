...
// ─── Types under test ─────────────────────────────────────────────────────────
import type { EmailMessage, EmailResult } from '../plugins/email/types.ts'
import { createUnconfiguredCampaign }     from '../plugins/email/campaign/unconfigured.ts'
import { createSystemSender }             from '../plugins/email/system/sender.ts'
import { SystemEmailError }               from '../plugins/email/system/sender.ts'
import { sendSystemEmail, sendCampaignEmail } from '../plugins/email/hook.ts'
...
describe('createSystemSender', () => {
  ...
  it('throws SystemEmailError when html and text are both missing', async () => {
    // Mock sendMail to avoid real SMTP
    const mod = await import('../plugins/email/system/smtp.ts')
    const orig = mod.sendMail
    ;(mod as Record<string, unknown>).sendMail = mock(async () => {})
    ...
    ;(mod as Record<string, unknown>).sendMail = orig
  })
  ...
  it('applies plugin-level from when message has none', async () => {
    ...
    ;(mod as Record<string, unknown>).sendMail = mock(async (_cfg: unknown, msg: { from: string }) => {
...
describe('createCampaignSender — Resend', () => {
  it('sends to /emails with Resend payload shape', async () => {
    const { createCampaignSender } = await import('../plugins/email/campaign/sender.ts')
...
describe('email plugin', () => {
  ...
  it('sets app.email on register()', async () => {
    const { email } = await import('../plugins/email/plugin.ts')
    const plugin = email({
