...
import { createLitestoneService, withLitestoneDb } from '../core/litestone.ts'
...
//
// When you need more control than createLitestoneService provides,
...
app.patch('/api/users/:id/avatar', async ctx => {
  ...
  const scopedDb = (ctx.params.db as typeof db)
...
app.post('/api/users/:id/avatar/presign', async ctx => {
  if (!ctx.params.user) return ctx.json({ message: 'auth required' }, 401)
