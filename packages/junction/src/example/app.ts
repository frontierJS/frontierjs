...
import type { WsContext }      from '../transport/types.ts'
...
app.configure(channels((a: App & { channels?: ReturnType<typeof import('../transport/channels.ts').createChannelManager> }) => {
...
app.get('/prices/{symbol}', ctx => {
  ...
  const interval = setInterval(async () => {
    ...
    try {
      ...
      const { bridge: b } = await import('../transport/bridge.ts')
      const svcCtx = b.internal('prices', 'get', null)
      svcCtx.id    = symbol
      const { callService } = await import('../core/service.ts')
