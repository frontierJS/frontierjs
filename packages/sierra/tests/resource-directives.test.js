 ...
 * the DIRECTIVES: { limit, offset, orderBy, select }. That second argument used to
...
describe('directives threading', () => {
  ...
  test('getOptions() falls back to optionsQuery.directives', async () => {
    const { service } = createResource({
      service: 'categories',
      optionsQuery: { query: { active: true }, directives: { orderBy: 'name', limit: 500 } },
    })
  ...
  test('an explicit getOptions() argument overrides optionsQuery', async () => {
    const { service } = createResource({
      service: 'categories',
      optionsQuery: { query: { active: true }, directives: { limit: 500 } },
    })
...
describe('directives on the hook context', () => {

  test('a before hook can set pagination', async () => {
    const { service } = createResource('leads', {
      hooks: { before: { find: [ctx => { ctx.directives.limit = 50 }] } },
    })
    await service.find({})
  ...
  test('ctx.locals stays client-side — it never reaches the proxy', async () => {
    const seen = []
    const { service } = createResource('leads', {
      hooks: {
        before: { find: [ctx => { ctx.locals.loading = true }] },
        after:  { find: [ctx => { seen.push(ctx.locals.loading) }] },
      },
    })
    await service.find({ status: 'new' })
    ...
    expect(_calls[0][2]).toEqual({})                 // directives — unpolluted
  })
})
