...
box submits `''` — so the resource's `blankToNull` is what stands between the second lead
...
| A route named `new.mesa` | Mesa named the component function after the file and sanitized invalid *characters* but not reserved *words* — `export default function new(…)`. It ran in dev and failed only at `vite build`. This route is `create.mesa` for that reason. **Fixed 2026-08-04**, along with the sibling case below; the name is now checked against reserved words and against module scope. |
...
| Numbers arrived as strings | `el.value` is a string for every control, `<select>` and `type="number"` included, and Mesa passes it through unchanged (correctly — it does not know the field). The form failed validation with *"value must be a number"*. Only the schema knows the types, so Sierra casts them, and has since. |
