...
## 2026-08-03 → 2026-08-19 — the build log
...
- **Neither screen executes anything itself.** Both queue on Caravan's `fleet`
  queue, and `api/src/jobs/recipe-run.job.ts` / `cleanup-run.job.ts` ask the
  outpost through `app.conduit`. A non-zero exit is recorded and never retried;
...
**This section describes work since finished.** `packages/outpost` now exists
and is what basecamp's deploy engine actually speaks to — a step against a
server with no outpost registered now refuses loudly by name (`api/src/providers/executor.ts`,
`recipe-run.job.ts`, `cleanup-run.job.ts`) rather than reporting a silent
success. The plan as it was ordered, for history:
...
*The invitation flow this used to track as `FJS-032` is built; **`FJS-031`**
(publishes on reads) is still open in `../../ISSUES.md`.*
...
6. **Build the remaining 16 screens** — `FJS-153`, phased in `docs/SCREENS.md`
   ...
   **Two things ruled before it starts** (2026-08-10, written up in
   ...
   which closes Phase 6's "a key belongs to whoever made it"). The invitation
   flow this tracked as `FJS-032` is built (`model Invitation`, mail through
   conduit, a signed-out acceptance route). `Blueprint` and `Backup` are
