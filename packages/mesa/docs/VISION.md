...
### CSS scoping — the class goes on the subject
...
```html
<button class="mHASH">click</button>
<style>button.mHASH { color: red }</style>
```

> **RULE 55** — a component's scoped CSS is emitted by appending `.mHASH` to the
> **rightmost compound selector** — the selector's subject — and every element
> in a component that has styles carries that class. A component therefore
...
- **You can style your own root.** Scoping used to emit `.mHASH button`, an
  *ancestor* selector, while putting the class *on* the element — and
  `.mHASH button` matches a `button` INSIDE a `.mHASH` element, never the
  `<button class="mHASH">` carrying it. So a rule targeting the component's own
  root silently did nothing. It was also only added to elements that already had
  a `class` attribute, so a bare `<button>` never got one at all. Both are fixed.
- **Your styles no longer leak into child components.** The ancestor form put
  every descendant of the component root in range, including the markup of any
  child component it rendered. The subject form cannot: a child's elements carry
  the child's hash. **This is a breaking change** for any component that was
  reaching into a child — use `:global(...)`, which exists for exactly this.

`:global(...)`, `:root`, `html` and `body` are emitted unscoped, unchanged. In a
nested rule, a compound containing `&` is already scoped through its parent and
is left alone — but the subject still gets scoped, so `& + p` becomes
`p.mHASH + p.mHASH` rather than leaving the trailing `p` free to match anything
adjacent on the page.

A component with no `<style>` block emits no scope class at all.
...
Mesa — the compiler rewrites the reads — and it is what makes a snippet update: its DOM is
built once, so an argument read as a plain value would be frozen at that moment, and a table
