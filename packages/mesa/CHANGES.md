...
reference, so the vocabulary is in the file: state and `$:`, the template, components talking to
each other, `$`, server and client, and a wrong-guesses table from Svelte 4/5, Vue and JSX habits
in which every row was compiled and, where the failure is at runtime, mounted. `files:` carries it.
The pass found `FJS-1109`, `FJS-1111` and `FJS-1112`.
...
## 2026-08-01 — the reactivity audit and the block-teardown pass

The two pass records, moved out of `docs/`. Their open items are `FJS-1142`, `FJS-1143` and `FJS-1144`; the two failure shapes and the claims that died under testing are worth reading before changing `runtime.js`.
