...
<button on:click={() => { base = 2; factor = 3; offset = 10 }}>Reset</button>`
  ...
  // Mutating a property silently does nothing. Replace the whole value.
...
-->`
  ...
  contextIsolation: {
    ...
    group: 'Context',
    src: `<script>
  // The key advantage of context over stores:
  // INSTANCE ISOLATION — each component instance has its own context.
  //
  // Two counters below each provide their own $context.count.
  // Clicking "double" in one doesn't affect the other.
  // A global store cannot do this — it would be shared between both.

  // This component IS the counter provider.
  // In a real app, Counter.mesa would have:
  //   let count = 0
  //   $context.count   = count
  //   $context.inc     = () => count++
  //   $context.double  = () => count = count * 2
  //   $context.reset   = () => count = 0

  // Simulating two independent counter instances side by side:
  let countA = 0, countB = 0

  function incA()    { countA++ }
  function doubleA() { countA = countA * 2 || 1 }
  function resetA()  { countA = 0 }

  function incB()    { countB++ }
  function doubleB() { countB = countB * 2 || 1 }
  function resetB()  { countB = 0 }
</script>

<h2>Context — instance isolation</h2>

<p style="font-size:12px;color:#6b7280;margin-bottom:16px">
  Two independent counter instances. Each has its own state.
  A global store would share state between them — context does not.
</p>

<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
  <!-- Instance A -->
  <div style="padding:16px;border:2px solid #6366f1;border-radius:8px">
    <div style="font-size:11px;color:#6b7280;margin-bottom:4px">Instance A</div>
    <div style="font-size:36px;font-weight:600;color:#6366f1;margin-bottom:8px">
      {countA}
    </div>
    <div style="display:flex;gap:6px;flex-wrap:wrap">
      <button on:click={incA}>+1</button>
      <button on:click={doubleA}>×2</button>
      <button on:click={resetA}>reset</button>
    </div>
  </div>

  <!-- Instance B -->
  <div style="padding:16px;border:2px solid #10b981;border-radius:8px">
    <div style="font-size:11px;color:#6b7280;margin-bottom:4px">Instance B</div>
    <div style="font-size:36px;font-weight:600;color:#10b981;margin-bottom:8px">
      {countB}
    </div>
    <div style="display:flex;gap:6px;flex-wrap:wrap">
      <button on:click={incB}>+1</button>
      <button on:click={doubleB}>×2</button>
      <button on:click={resetB}>reset</button>
    </div>
  </div>
</div>

<p style="font-size:11px;color:#9ca3af;margin-top:16px">
  In a real Mesa app, each box would be a &lt;Counter&gt; component that
  provides $context.count, $context.inc etc. to its own subtree.
  Child components inside would read via: const count = $context.count
</p>`
  },
...
{/if}`,
  ...
  uiComponents: {
    ...
    group: 'UI Library',
    src: `<script>
  import Button from '@ui/Button.mesa'
  import Badge  from '@ui/Badge.mesa'
  import Card   from '@ui/Card.mesa'
  import Input  from '@ui/Input.mesa'
