/*
 * A snippet argument crossing a component boundary, and the table shape it
 * showed up in.
 *
 * Both cases are one question asked twice: a value that reaches its render site
 * through somebody ELSE's slot or `{@render}` still tracks. The first is a
 * snippet arg inside a nested component's `<slot>`; the second is the shape it
 * was found in — an `{#each}` inside an `{:else}` inside a `<table>`, where a
 * row is a snippet the parent passes down.
 *
 * The file was called `tmp2.test.js` for its whole life and is a real suite: a
 * scratch name is how a suite ends up in no table and read by nobody (FJS-888).
 */
