 ...
 *
 * Run with:
 *   npx vitest run --environment happy-dom compiler.test.js
 *
 * Sections
...
import { setRenderEnvironment } from '../src/runtime.js'
...
{provide('header', header)}{provide('footer', footer)}`
  ...
  describe('component instantiation', () => {
    ...
    it('class on HTML elements is NOT remapped', async () => {
      ...
      // Every dynamic class goes through the merging binder, not
      // bindAttribute/set_attribute — see the scope-class test below.
      expect(out).toContain('bindClassPassthrough')
    })

    // A dynamic class attribute used to be emitted as
    // `set_attribute(el, 'class', …)`, which REPLACES the attribute — and the
    // scope class RULE 55's selectors match lives in the template. So an
    // element whose class contained an interpolation lost every style its own
    // component declared for it. Nothing threw and nothing warned: the
    // component rendered, with the author's classes and none of their CSS.
    it('a dynamic class keeps the component scope class', async () => {
      const ctx = await compile(
        `<script>let theme='blue'</script><div class="card {theme}">x</div>` +
        `<style>.card { color: red } .blue { background: blue }</style>`,
        // css:false hands the scoped CSS back on ctx.css.result instead of
        // emitting an addStyles call — the same option the REPL uses.
        { debug: false, css: false })
      const id = ctx.css.id
      expect(id).toBeTruthy()
      // The template carries the scope class...
      expect(ctx.result).toContain(`class="${id}"`)
      // ...and the binder merges rather than replaces, so it survives.
      expect(ctx.result).toContain('bindClassPassthrough')
      expect(ctx.result).not.toMatch(/set_attribute\([^,]+, ?'class'/)
      // The CSS is scoped onto the selector's subject, so it only matches if
      // the element still carries the class after the binder has run.
      expect(ctx.css.result).toContain(`.card.${id}`)
      expect(ctx.css.result).toContain(`.blue.${id}`)
    })

    // <style> is extracted from the top level only. A nested one was neither
    // collected as component CSS nor left in the template — it vanished, with
    // no error and no warning, and a REPL example shipped a spinner whose
    // @keyframes never existed.
    // {@debug n} and {@nonsense n} compiled to byte-identical output — the tag
    // was dropped at a branch whose comment read "other @directives fall
    // through silently". A typo cost you markup and told you nothing.
    it('warns on an unknown {@tag} and names the known ones', async () => {
      const typo = await compile(
        `<script>let n = 1</script>{@rendr thing()}<p>{n}</p>`, { debug: false, css: false })
      expect(typo.analysis.warnings.join(' ')).toMatch(/Unknown template tag \{@rendr/)
      expect(typo.analysis.warnings.join(' ')).toMatch(/\{@render\}/)
    ...
    it('stays silent for every tag Mesa does support', async () => {
      const cases = [
        `<script>let h = '<b>x</b>'</script>{@html h}`,
        `<script>let n = 1</script>{#snippet row(x)}<p>{x}</p>{/snippet}{@render row(n)}`,
        `<script>let n = 2</script>{@const d = n * 2}<p>{d}</p>`,
        // {@attach} lives inside a tag, so it is an attribute and never reaches
        // the systag branch — the warning must not fire for it either.
        `<script>let n = 1; function f(el) {}</script><div {@attach f}>{n}</div>`,
      ]
      for (const src of cases) {
        const out = await compile(src, { debug: false, css: false })
        expect(out.analysis.warnings, src).toEqual([])
      }
    })

    it('warns when a <style> block is nested inside a block', async () => {
      const nested = await compile(
        `{#if true}<style>div { color: red }</style>{/if}`, { debug: false, css: false })
      expect(nested.analysis.warnings.join(' ')).toMatch(/top level of the component/)
      expect(nested.css.result).toBeFalsy()

      const snippet = await compile(
        `{#snippet s()}<style>div { color: red }</style>{/snippet}`, { debug: false, css: false })
      expect(snippet.analysis.warnings.join(' ')).toMatch(/top level of the component/)

      // Top level is the supported position and stays silent.
      const ok = await compile(
        `<div>x</div><style>div { color: red }</style>`, { debug: false, css: false })
      expect(ok.analysis.warnings).toEqual([])
      expect(ok.css.result).toContain('color: red')
    })

    it('a dynamic class still carries the scope class in the DOM', async () => {
      const runtime = await import('../src/runtime.js')
      const ctx = await compile(
        `<script>let theme='blue'</script><div class="card {theme}">x</div>` +
        `<style>.card { color: red }</style>`,
        { debug: false, css: false })
      const Comp = execCompiled(ctx.result, runtime)
      const div = mount(Comp, runtime).find('div')
      expect(div.classList.contains('card')).toBe(true)
      expect(div.classList.contains('blue')).toBe(true)
      expect(div.classList.contains(ctx.css.id)).toBe(true)
    })

    // RULE 55's cross-component consequence, and the reason the REPL's
    // classSystem example had to change: passing a class into a child delivers
    // the NAME, but the parent's rule is scoped to the PARENT's hash and the
    // child's elements never carry it. The markup looks right and nothing is
    // styled. `:global(...)` is the documented escape.
    it('a class handed to a child is NOT reached by the parent\'s scoped rule', async () => {
      const runtime = await import('../src/runtime.js')
      const Chip = await compileAndExec(
        `<script>export let label = ''</script><span {class}>{label}</span>`, runtime)

      const scoped = await compile(
        `<script>import Chip from './Chip.mesa'</script><Chip label="x" class="chip" />` +
        `<style>.chip { background: #dcfce7 }</style>`,
        { debug: false, css: false })
      const id = scoped.css.id
      expect(scoped.css.result).toBe(`.chip.${id} { background: #dcfce7 }`)

      const P = execCompiled(scoped.result, runtime, { './Chip.mesa': { default: Chip } })
      const span = mount(P, runtime).find('span')
      expect(span.className).toBe('chip')             // the name arrives…
      expect(span.matches(`.chip.${id}`)).toBe(false) // …the rule cannot match it

      // :global drops the hash, so the same rule reaches the child.
      const global_ = await compile(
        `<script>import Chip from './Chip.mesa'</script><Chip label="x" class="chip" />` +
        `<style>:global(.chip) { background: #dcfce7 }</style>`,
        { debug: false, css: false })
      expect(global_.css.result).toBe('.chip { background: #dcfce7 }')
      const P2 = execCompiled(global_.result, runtime, { './Chip.mesa': { default: Chip } })
      expect(mount(P2, runtime).find('span').matches('.chip')).toBe(true)
    })
  })
  ...
  describe('$.context', () => {
    ...
    // that one calls the component directly, which registers no delegation
    // root, so no click would ever fire.
    const mountLive = (Comp, runtime) => {
      const c = document.createElement('div')
      document.body.appendChild(c)
      const anchor = document.createComment('')
      c.appendChild(anchor)
      runtime.mount(anchor, Comp, { props: {}, root: c })
      runtime.flushSync()
      return c
    }
    ...
    it('a child re-reads $.context when the provider changes', async () => {
      ...
      const parent = await compile(
        ...
        { debug: false, css: false })
      const c = mountLive(
        execCompiled(parent.result, runtime, { './Child.mesa': { default: Child } }), runtime)

      expect(c.querySelector('b').textContent).toBe('0')
      c.querySelector('button').click(); runtime.flushSync()
      expect(c.querySelector('b').textContent).toBe('1')
      c.querySelector('button').click(); runtime.flushSync()
      expect(c.querySelector('b').textContent).toBe('2')
    })
    ...
    it('two instances of a provider hold independent context', async () => {
      ...
      const app = await compile(
        `<script>import Box from './Box.mesa'</script><Box /><Box />`,
        { debug: false, css: false })
      const c = mountLive(
        execCompiled(app.result, runtime, { './Box.mesa': { default: Box } }), runtime)

      const vals = () => [...c.querySelectorAll('i')].map((e) => e.textContent)
      expect(vals()).toEqual(['0', '0'])
      c.querySelectorAll('button')[0].click(); runtime.flushSync()
      c.querySelectorAll('button')[0].click(); runtime.flushSync()
      expect(vals()).toEqual(['2', '0'])          // the second instance is untouched
      c.querySelectorAll('button')[1].click(); runtime.flushSync()
  ...
  describe('#snippet + @render', () => {
    ...
    it('{@render} calls snippet with anchor and args, each arg as a getter', async () => {
      // An argument is passed as `() => value`, not as the value: the snippet's
...
describe('scopeCSS', () => {
  ...
  it('scopes a simple element selector', () => {
    expect(scope('p { color: red; }')).toContain('p.mesa-test')
  })
  it('scopes a class selector', () => {
    expect(scope('.card { color: red; }')).toContain('.card.mesa-test')
  })
  it('scopes multiple selectors in a rule', () => {
    const out = scope('h1, h2, p { color: red; }')
    expect(out).toContain('h1.mesa-test')
    expect(out).toContain('h2.mesa-test')
    expect(out).toContain('p.mesa-test')
  })
  ...
  it(':global(sel) strips wrapper and emits unscoped', () => {
    ...
    expect(out).not.toContain('body.mesa-test')
  })
  ...
  it(':is() — commas inside parens not treated as selector list', () => {
    ...
    expect(out).not.toContain('h1.mesa-test')
    expect(out).not.toContain('h2.mesa-test')
  })
  ...
  it(':has() — passes through correctly', () => {
    ...
    expect(out).toContain('p:has(> span).mesa-test')
  })
  ...
  it('@media — scopes rules inside, not the @media itself', () => {
    ...
    expect(out).toContain('p.mesa-test')
    expect(out).not.toContain('@media.mesa-test')
  })
  ...
  it('deep nesting — scopes each level', () => {
    ...
    expect(out).toContain('div.mesa-test')
    expect(out).toContain('p.mesa-test')
    expect(out).toContain('span.mesa-test')
  })
  ...
  it('multiple & selectors in one block', () => {
    ...
    expect(out).toContain('p.mesa-test.active')
    // `& + p` — the SUBJECT is the trailing p, and it needs its own scope or it
    // matches any adjacent p on the page.
    expect(out).toContain('p.mesa-test + p.mesa-test')
  })
  ...
  it('comment before selector — comment preserved, selector scoped', () => {
    ...
    expect(out).toContain('p.mesa-test')
    expect(out).not.toContain('/* heading */.mesa-test')
  })
...
describe('mesa:boundary', () => {
  ...
  beforeEach(async () => {
    runtime = await import('../src/runtime.js')
...
<button id="btn" on:click={toggle}>toggle</button>`, runtime, imports)
  ...
  // The same idiom on a LOCAL reactive binding. It compiled to an ordinary
  // `$$set_user(user)`, and a signal writes through Object.is, so assigning the
  // identical reference was skipped and nothing happened — `x = x` worked for an
  // imported object (below) and silently no-opped for a local one.
  it('self-assignment on a local let forces a notify', async () => {
    const runtime = await import('../src/runtime.js')
  ...
  let user = { score: 0 }
  function bump() { user.score += 10; user = user }
</script><p>{user.score}</p><button on:click={bump}>go</button>`, runtime)
    const c = document.createElement('div')
    document.body.appendChild(c)
    const anchor = document.createComment('')
    c.appendChild(anchor)
    ...
    c.querySelector('button').click(); runtime.flushSync()
    expect(c.querySelector('p').textContent).toBe('10')
    c.querySelector('button').click(); runtime.flushSync()
    expect(c.querySelector('p').textContent).toBe('20')
  })
  ...
  it('an equal write is still skipped; only force overrides it', async () => {
    const runtime = await import('../src/runtime.js')
    const [read, write] = runtime.createSignal(1)
    let runs = 0
    runtime.createRoot(() => { runtime.createEffect(() => { read(); runs++ }) })
    runtime.flushSync()
    expect(runs).toBe(1)
    write(1); runtime.flushSync()
    expect(runs).toBe(1)          // equal — skipped, as before
    write(1, true); runtime.flushSync()
    expect(runs).toBe(2)          // forced — notifies
    write(2); runtime.flushSync()
    expect(runs).toBe(3)          // ordinary change — unaffected
  })
...
<button id="btn" on:click={clearPost}>clear</button>`, runtime)
  ...
  describe('CSS scoping — document-root selectors', () => {
    ...
    it('component class selectors stay scoped', async () => {
      ...
      expect(css._rules[0]).toMatch(/\.card\.[a-z0-9]+ \{/)
    })
...
describe('dev instrumentation — runtime', () => {
  it('__dev.r() registers signal and associates with current component', async () => {
    const { __dev, track, set, push_component, pop_component } = await import('../src/runtime.js')
    // Clear between tests
  ...
  it('snapshot() returns current state', async () => {
    const { __dev, track, push_component, pop_component } = await import('../src/runtime.js')
    __dev._signals.clear(); __dev._components.clear(); __dev._log = []
  ...
  it('__dev has zero overhead when no signals registered (prod path)', async () => {
    const { __dev, track, set } = await import('../src/runtime.js')
    __dev._signals.clear(); __dev._components.clear(); __dev._log = []
