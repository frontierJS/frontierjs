 ...
 * the module-scope collision check lives beside the emit site.
 * `let`/`static`/`yield` are only reserved in strict mode — a module always is.
 */
 ...
 *    It cost a false alarm of 13 "differences" when checking that a compiler
 *    change was byte-identical.
 * 2. **One component, two compilers, one id.** A prerendered island is compiled
 *    by Mesa's renderer AND by Vite for its client chunk. Under two ids, the
 *    page carried two copies of the same rules under two hashes, and the markup
 *    swapped class on mount. Under one id, `addStyles` finds the prerendered
 *    `<style id=…>` already in the head and injects nothing.
 * 3. **Debuggability.** A scope class that changes on every build is not
 *    something you can search for.
 *
 * Hashing the STYLE CONTENT and nothing else is deliberate. Adding the filename
 * would break (2) the moment the two compilers disagree about a path — an
 * absolute path, a Vite id with a query string, a symlinked workspace — and
 * that failure would be silent. The cost is that two components with
 * byte-identical CSS share an id, which is harmless: their rules are the same
 * rules, so applying either to both changes nothing.
 *
 * FNV-1a in two lanes, base36. No dependency and no `node:crypto`, because the
 * compiler also runs in the browser (the REPL imports it).
 */
export function cssHash(content) {
  let h1 = 0x811c9dc5
  let h2 = 0x01000193
  ...
  // Leading 'm' keeps it a valid CSS identifier whatever the digits do.
  return 'm' + ((h1 >>> 0).toString(36) + (h2 >>> 0).toString(36)).slice(0, 10)
}
...
xNode.node = (data) =>
  xNode.baseNode(
    ...
    (ctx, node) => {
      ...
      // not just the ones that already had a `class` attribute.
      //
      // Scoping now appends `.hash` to the rightmost compound selector, so the
      // class has to be present on whatever the selector's subject is — and
      // that is routinely an element the author never gave a class to
      // (`button { … }` over `<button>`). Gating on `node.class.size` meant
      // those rules matched nothing at all.
      const _css = get_context().css
      if (node.class.size || _css?.active?.())
        ctx.add(_css.resolveAsNode(node.class, [' class="', '"']))
...
/**
 * Pseudo-elements must stay last in a compound selector — `a::before.hash` is
 * invalid, `a.hash::before` is what was meant. The single-colon legacy spellings
 * are pseudo-elements too, and are the reason this cannot just look for `::`.
 */
const _PSEUDO_ELEMENT = /::|:(?:before|after|first-line|first-letter|placeholder|backdrop|marker|selection|file-selector-button)\b/

/**
 * Append `.cssId` to the RIGHTMOST compound selector.
 *
 * This is the whole scoping strategy, and it used to be the opposite: selectors
 * were emitted as `.cssId <selector>`, an ANCESTOR selector, while the class was
 * put ON the element. Those two cannot both be true —
 * `.hash button` matches a `button` INSIDE a `.hash` element, and never the
 * `<button class="hash">` that carries it. So a component could not style its
 * own root, silently: the rule was emitted, the class was emitted, and nothing
 * matched. Confirmed in Chrome before changing it.
 *
 * The rightmost form also stops styles leaking INTO child components, which the
 * ancestor form allowed — a child's elements carry the child's id, not this
 * one's. Reach across a component boundary with `:global(...)`, which is what
 * it is for.
 */
function _appendScope(sel, cssId) {
  // Find the last top-level combinator, so we scope only the final compound.
  let depth = 0, cut = -1
  ...
  const head = cut === -1 ? '' : sel.slice(0, cut + 1)
  const tail = cut === -1 ? sel : sel.slice(cut + 1)
  if (!tail) return sel   // trailing combinator — malformed, leave it alone

  // A compound containing `&` is already scoped, because `&` expands to the
  // parent selector and the parent was scoped when it was emitted. Appending
  // again would just produce `p.hash:hover.hash`.
  //
  // The subject is what matters, not the whole selector: in `& + p` the `&`
  // sits on the LEFT and the subject `p` carries no scope at all, so it matched
  // any adjacent `p` on the page, including outside the component. Scoping the
  // rightmost compound is what closes that.
  if (tail.includes('&')) return sel

  // Insert before a pseudo-element if the compound has one.
  const m = tail.match(_PSEUDO_ELEMENT)
  if (m && m.index > 0) {
    return `${head}${tail.slice(0, m.index)}.${cssId}${tail.slice(m.index)}`
  }
  if (m && m.index === 0) {
    // A bare `::before` with no subject — scope the element it hangs off, which
    // is whatever preceded the combinator; nothing sensible to do here.
    return `${head}${tail}`
  }
  return `${head}${tail}.${cssId}`
}
...
function _scopeSelector(sel, cssId) {
  ...
  // & nesting ref — handled inside _appendScope, which scopes the SUBJECT.
  // Returning early here left `& + p`'s subject unscoped, so it matched any
  // adjacent `p` on the page.
    ...
    body.forEach((n) => {
      if (n.type === 'text') {
        ...
        if (n.value.startsWith('@render ')) {
          ...
          // comment reading "other @directives fall through silently". A typo
          // ({@rendr items}) or a Svelte habit ({@debug x}) compiled clean,
          // emitted nothing, and left you hunting for markup that was never
          ...
          ctx.analysis.warnings.push(
            ...
            `Known tags are {@render}, {@html} and {@const}; {@attach} goes inside ` +
            `an element's tag, not on its own.${hint}`
          )
        }
 ...
 *
 * `{@render row(order)}` passes `() => order`, and inside the body `order`
 * resolves through `order()`. A snippet's DOM is built once and then kept up to
 * date by the same effects everything else uses, so an argument that arrives as
 * a plain value is read once and frozen — the caller's next value never reaches
 * the markup. That is what made a kit Table draw its first set of rows and then
 * ignore the store: no error, no re-render, just stale cells.
 *
 * Reading through a getter keeps the fine-grained model. The read happens
 * inside the binding's own effect, so it subscribes to whatever signal the
 * caller's expression touched.
 */
export function makeSnippet(data) {
  const ctx = this
  const { name, rawArgs, body } = data
  // A snippet declared inside a component tag is emitted under a unique name —
  // it is passed as a prop, not resolved by {@render}, and two components in
  ...
  return xNode('snippet-def', { name, varName, argNames, block }, (w, n) => {
    ...
    w.writeLine(`const ${n.varName} = (${argList}) => {`)
    w.indent++
...
export function makeComponent(node, option = {}) {
  ...
  //
  // VISION §9.5: "named snippets defined directly inside a component tag are
  ...
  //
  //   <Table {columns} {rows}>{#snippet row(r)}<tr>…</tr>{/snippet}</Table>
  //
  ...
  for (const s of snippetChildren) {
    ...
    // block, so it needs no push effect. What it renders stays reactive because
    // the block it builds reads the parent's signals.
    allProps.push({ name: s.name, value: varName, isStatic: true })
  }
  ...
  //
  // A prop name is an attribute name, and plenty of those are not identifiers:
  // `aria-label`, `data-id`, `aria-describedby`. Emitting them bare produced
  // `{aria-label: '…'}` — a syntax error in generated code, with nothing said
  // at compile time.
  ...
  const buildPropsObj = (props, spreads) => {
    ...
    if (spreads.length) {
      const staticObj = props.length ? `{${props.map(p => `${key(p.name)}: ${p.value}`).join(', ')}}` : '{}'
      const parts = ['{}', ...spreads.map(s => `(${s})`), staticObj]
      return `Object.assign(${parts.join(', ')})`
    }
    return `{${props.map(p => `${key(p.name)}: ${p.value}`).join(', ')}}`
  }
  ...
  const makeBind = (anchorName) => xNode(
    ...
    { anchorName, componentName, allProps, reactiveProps, spreadProps, twoWayProps,
      slots: slotBlocks, snippetDefs, bindThisSetter, hasSlots, hasReactive, hasSpreads,
      hasBindThis, island },
    (w, n) => {
      // Snippet props are defined first: the call that receives them is on the
      // next line, in the same scope.
      for (const def of n.snippetDefs) w.add(def)
...
export function bindProp(prop, node, element) {
  ...
  if (prop.value && (prop.value.includes('{') || prop.type === 'exp')) {
    ...
    // `class` merges into the element's existing classes; every other attribute
    // replaces. Routing it through bindAttribute overwrote the whole `class`,
    // so `<button class="btn primary" {class}>` rendered with no classes at all
    // unless a class prop happened to be passed — and with only the consumer's
    // when one was. Kept out of the bindAttribute grouping pass on purpose:
    // that pass rewrites those calls into set_attribute.
    //
    // This covers EVERY dynamic class, not just the {class} passthrough. A
    // scoped component puts its scope class in the template — that is what
    // RULE 55's `.card.mHASH` selectors match — and `set_attribute(el,'class',…)`
    // replaced it, so an element whose class contained an interpolation
    // (`<div class="card {theme}">`, which is what any variant API looks like)
    // silently lost every one of its own component's styles. The static classes
    // are folded into the expression, so the only thing the template contributes
    // here is the scope class; merging is exactly right.
    if (name === 'class') {
      return {
...
export function inspectProp(prop) {
  ...
  // A valueless attribute on a component is the BOOLEAN form — `<Table striped>`
  // means striped={true}, exactly as `<input disabled>` means disabled. The
  // shorthand that passes a variable of the same name is the braced `{striped}`,
  // which the parser has already turned into a type:'exp' prop by the time it
  // gets here.
  //
  // This used to emit the attribute NAME as an expression, so `<Table striped>`
  // compiled to `striped: striped` — a reference to a variable the caller never
...
export function emitScript(ctx) {
  ...
  sorted.forEach((v) => {
    ...
    if (v.kind === 'var') {
      ...
      //
      // The LOOKUP is hoisted out of the derived on purpose. `contextRead`
      // walks a stack that only exists while a component is being constructed
      // (between push_component and pop_component). A derived recomputes later,
      // from an effect, when that stack is empty — so a lookup inside the body
      // resolved on the first read and returned null on every one after it, and
      // the value went `undefined` the moment the provider changed. Reactivity
  ...
  ctx.analysis.contextProvides.forEach(({ key, initRaw, initNode }) => {
    // rewriteAssignments FIRST — see the derived-const and $: emitters. Providing
    // a mutator through context (`$context.toggle = (id) => { open = … }`) is the
    // normal way a compound component shares its state, and it emitted an invalid
    // assignment target until this was added.
    const rewritten = rewriteExpr(
      initNode ? rewriteAssignments(initRaw, initNode, ctx) : initRaw,
      ctx.accessors
    )
  ...
  ;(postCallHooks ?? []).forEach((hook) => {
    ...
    // The hook works by REPLACING the binding — `fn = (...args) => { … }` — so
    // the name has to be assignable. A `const` (very often a destructured one,
    // `const { get: rows } = useStore(store)`) compiled fine and threw
    // `Assignment to constant variable` on mount, before anything rendered.
    // Say so at compile time instead.
    const target = ctx.analysis?.vars?.[fnName]
    if (target && (target.kind === 'const' || target.isImport)) {
      ctx.analysis.errors.push(
        `$: ${fnName}(), handler — '${fnName}' is a ${target.isImport ? 'import' : 'const'} and cannot be ` +
        `wrapped. A post-call hook replaces the binding, so it needs a \`function\` ` +
        `declaration or a \`let\`. To react to a VALUE instead, watch it: \`$: ${fnName}(), …\` ` +
        `becomes \`$: someValue, () => { … }\`.`
      )
      return
    }
...
function _isReactive(expr) {
  ...
  // written ONCE. In @frontierjs/ui's Steps that was `aria-current`: the step's
  // CLASS tracked the state (a class binding is always an effect) while the
...
export async function compile(source, config = {}) {
  ...
  function safeComponentIdent(name, ctx) {
    ...
    let n = 2
    while (RESERVED_WORDS.has(candidate) || moduleScopeNames(ctx).has(candidate)) {
      candidate = `${name}_Component${n++}`
    }
    return candidate
  }
  ...
  function moduleScopeNames(ctx) {
    ...
    if (src) {
      let ast
      try {
        ast = acorn.parse(src, { sourceType: 'module', ecmaVersion: 'latest' })
      } catch {
        ast = null            // a module script that will not parse is reported elsewhere
      }
      for (const node of ast?.body ?? []) {
        const collect = (d) => {
          if (!d) return
          if (d.type === 'Identifier') names.add(d.name)
          else if (d.type === 'ObjectPattern') d.properties.forEach(p => collect(p.value ?? p.argument))
          else if (d.type === 'ArrayPattern') d.elements.forEach(collect)
          else if (d.type === 'AssignmentPattern') collect(d.left)
          else if (d.type === 'RestElement') collect(d.argument)
        }
        if (node.type === 'VariableDeclaration') node.declarations.forEach(d => collect(d.id))
        else if (node.type === 'FunctionDeclaration' || node.type === 'ClassDeclaration') collect(node.id)
        else if (node.type === 'ImportDeclaration') node.specifiers.forEach(s => collect(s.local))
        else if (node.type === 'ExportNamedDeclaration') {
          const d = node.declaration
          if (d?.type === 'VariableDeclaration') d.declarations.forEach(x => collect(x.id))
          else if (d) collect(d.id)
          node.specifiers?.forEach(s => collect(s.exported))
        }
      }
    }
    ctx._moduleScopeNames = names
    return names
  }
  ...
  //   specifier?: string,         // the module it was imported from, verbatim
  // }>
  //
  // `specifier` exists so a consumer can get from a marker's component NAME
  // back to a module to import. The compiler is the only place that knows the
  // mapping — it has the import declarations right here — and the alternative
  // is every meta-framework re-parsing the source with its own regex, which
  // would also get `import Counter as Tally from …` wrong.
  const _importedFrom = new Map()
  for (const imp of ctx.analysis?.imports ?? []) {
    for (const s of imp.specifiers ?? []) {
      if (s.local?.name) _importedFrom.set(s.local.name, imp.source?.value)
    }
  }
  ctx.islands = [];
    ...
    for (const n of (Array.isArray(nodes) ? nodes : [nodes])) {
      ...
      if (n.type === 'node' && n.name && /^[A-Z]/.test(n.name)) {
        ...
        if (clientAttr) {
          ...
          const spec = _importedFrom.get(n.name)
          if (spec) entry.specifier = spec
  ...
  ctx.result = use_context(ctx, () => {
    ...
    //                    plus `export default function leads(…)` — redeclaration
    //
    // The second became the common case once resources moved to .mesa (repo
    // invariant 18): a resource file is named after the thing it exports, so
    // orders.mesa exporting `orders` is the obvious thing to write. Both failed
...
function compactDOM(dom, full) {
  const walk = (node) => {
    ...
    node.body = node.body.filter((n) => {
      ...
      // A text node carrying content: a newline between two words is
      // WHITESPACE, and the old rule (`/\n\s*/g` → '') welded them together.
      // `its 401s and its\n  400s from it` rendered as `its400s from it`, and
      // `one <code>two</code>\n  three` as `two three` with no space — every
      // wrapped paragraph in every .mesa file, with nothing wrong-looking in
      // the source. Collapse to a single space, which is what HTML says a run
      // of whitespace means.
      else if (n.value.trim()) n.value = n.value.replace(/\s+/g, ' ')
      // Whitespace-only nodes keep the old rule exactly: a newline run goes
      // (that is the indentation between block elements), a bare space between
      // two inline elements stays. The DOM traversal counts these nodes, so
      // changing which of them survive is a desync bug — see refer() above.
