...
const FIELD = [
  ...
  t('map', 'field', 'identity', '("column_name")',
    ...
    { seeAlso: ['map'] }),
  t('default', 'field', 'identity', '(value | [value, …] | now() | uuid() | ulid() | cuid() | nanoid() | auth().<claim> | autoincrement())',
  ...
  t('sealed', 'field', 'protect', '',
    ...
    { context: 'enum DocState {\n  draft\n  issued\n}\n\nmodel InvoiceLine {\n  id        Int @id\n  exampleId Int\n  example   Example @relation(fields: [exampleId], references: [id])\n}',
      extraFields: 'state DocState @default(draft)\n  @@transitions(state, issue: draft -> issued @seals)',
...
const MODEL = [
  ...
  t('unique', 'model', 'shape', '([field, …][, nullsDistinct: true | where: <expr>])',
  ...
  t('transitions', 'model', 'access', '(<field>, [<name>:] <from>|[<from>,…] -> <to> [@gate(N)] [@system] [@seals], …)',
