...
export const POSITION_RULES = {
  typeField:  { excludes: ['id', 'unique', 'map', 'relation', 'generated', 'from',
                           ...
                           'values', 'scale', 'money', 'big'] },
  traitField: { excludes: ['id'] },
  traitBlock: { excludes: ['id', 'map', 'db', 'fts'] },
...
const TOP = [
  ...
  t(
    ...
    'schema',
    'declare',
    '<name> { path · driver · replication · retention · maxSize · model }',
...
const FIELD = [
  // identity
  t('id', 'field', 'identity', '', 'The primary key. One per model.', 'id Int @id'),
  t(
    'unique',
    'field',
    'identity',
  ...
  t(
    ...
    'field',
    'identity',
    '(value | [value, …] | now() | uuid() | ulid() | cuid() | nanoid() | auth().<claim> | autoincrement())',
  ...
  t(
    ...
    {
      context: 'model User { id Int @id }',
      extraFields: 'authorId Int',
      values: [
        vals(
          'onDelete',
  ...
  t(
    ...
    'field',
    'relate',
    '(<Model>, last|first|count|sum|max|min|exists: … [, where][, orderBy][, via][, withDeleted][, withTemplates])',
  ...
  t(
    ...
    'field',
    'relate',
    '(ref: <Model>[, key][, as][, onMissing: error|skip])',
    ...
    {
      context: 'model Team { id Int @id }',
      seeAlso: ['scoped'],
      values: [
        vals('onMissing', ['error', 'skip'], 'role String @edge(ref: Team, onMissing: %s)')
      ]
    }
  ),
  t(
  ...
  t(
    ...
    { seeAlso: ['guarded', 'transient', 'immutable'] }
  ),
  t(
    'immutable',
    'field',
    ...
    { seeAlso: ['system', 'guarded', 'version'] }
  ),
  t(
    'sealed',
    'field',
    ...
    'lines InvoiceLine[] @sealed',
    {
      ...
      seeAlso: ['transitions', 'immutable', 'keep']
    }
  ),
  t(
    'capability',
...
const MODEL = [
  ...
  t(
    ...
    '@@id([orgId, userId])',
    { extraFields: 'orgId String\n  userId String', omitId: true, seeAlso: ['id', 'unique'] }
  ),
  t(
    'unique',
    'model',
    'shape',
    '([field, …][, nullsDistinct: true | where: <expr>])',
  ...
  t(
    ...
    'model',
    'shape',
    '([field, …][, tokenize: unicode61|ascii|porter|trigram])',
  ...
  t(
    ...
    'model',
    'access',
    "('read'|'create'|'update'|'delete'|'all', <expression>[, message])",
  ...
  t(
    ...
    'model',
    'access',
    '(<field>, [<name>:] <from>|[<from>,…] -> <to> [@gate(N)] [@system] [@seals], …)',
...
export const DOCS = {
  ...
  'schema:claim': 'access-control.md',
 ...
 *
 * One axis, and it is REACH rather than difficulty: how many apps need this at
 * all. Difficulty is already carried by the blurb, the note and the docs page,
 * and a column holding both answers neither — `@@index` is wanted by nearly
 * every app and needs a query plan read correctly, so a mixed axis has nowhere
 * to put it.
 *
 *   essential   — every app has one. `fli new` and `fli scaffold` write these.
 *   common      — many apps reach for it, and a first week meets it.
 *   situational — a specific need: adoption, tenancy, documents, a shape.
 *
 * Hand-maintained on purpose. It is a judgment about readers, so nothing here
 * derives it and the test does not grade the CHOICE — it grades that a choice
 * was made: a word in no tier fails, a word in two fails, and a key naming a
 * word that is gone fails. A new attribute is untiered until somebody says.
 */
export const TIERS = {
  essential: [
    // declarations
 ...
 * Measured against the search (word, blurb and arity): `sum` finds @from,
 * `aggregate` and `rollup` found nothing at all — so the gap is vocabulary and
 * not the language. A synonym is a fact about what people type, which is the
 * one fact here that cannot be derived from the schema, the parser or the
 * catalog: it is stated, and graded instead.
 *
 * Four rules, all executed in test/catalog.test.ts. A key must name a row that
 * exists; a synonym must not BE a word (it would shadow the real one); it must
 * belong to one row only (two owners is a coin toss); and the existing search
 * must NOT already find it — a synonym the blurb already carries is dead weight
 * that reads as coverage. What no rule can grade is a synonym pointing at the
...
export const SYNONYMS = {
  ...
  'field:derived':     ['calculated', 'formula'],
  'field:computed':    ['virtual'],
  'field:sequence':    ['autoincrement', 'serial', 'nextval'],
  ...
  'field:generated':   ['stored column'],
  'field:check':       ['invariant'],
  'field:capability':  ['feature flag', 'entitlement'],
  ...
  'schema:tenancy':    ['multi-tenant', 'saas'],
  'schema:claim':      ['token claim'],
}

/** What a person might type for this word, or an empty list. */
export function synonymsFor(row) {
  return SYNONYMS[`${row.level}:${row.word}`] ?? []
}

/** The row a typed synonym names, exactly. `aggregate` is @from and nothing
 *  else, which is what the one-owner rule buys. */
export function bySynonym(text) {
  const q = String(text).trim().toLowerCase()
  const key = Object.keys(SYNONYMS).find(k => SYNONYMS[k].includes(q))
  if (!key) return null
  const [level, word] = key.split(':')
  return CATALOG.find(r => r.level === level && r.word === word) ?? null
}
