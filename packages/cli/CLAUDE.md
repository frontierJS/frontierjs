...
- **`app:atlas` renders a model junction computes; it reads no app itself.** One
  ...
  **The entry is never probed.** `core/app-entry.js` reads it off the committed
  `surface.snapshot.md`'s own `generated by:` header — the same line the
  `snapshots` phase reruns, from the same directory — so an app booting from its
  root or naming an `--export` is read correctly rather than guessed at; the
  command travels as argv against an allow-listed binary, because it comes out of
  a file. **There is deliberately no per-service *what graded it* column**:
