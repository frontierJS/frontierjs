...
// graded the way `fli check`'s `proof-target` grades it, off the same function,
// so the page and the rule cannot disagree about what is missing.
//
// An empty runnable list degrades honestly rather than throwing — a target then
// grades as `script` where it would have been a listed row, and `unknown` is
// unaffected, which is the only grade this page reports on.
function proofs(root) {
  let rows = []
  try { rows = runnables(root) } catch { rows = [] }

  return readProofs(root).map(p => ({
    changed: plain(p.changed),
    run:     plain(p.run),
    targets: resolveRun(p.run, { root, rows })
      .map(t => ({ name: t.name, where: t.where, kind: t.kind })),
  }))
}
...
// no section at all: the report in any other monorepo printed `example` at
...
function decisions(root) {
  ...
  // newest first, so the first ruling under one is the current thinking.
  const sections = []
  const byTitle  = new Map()

  for (const r of readRegisters(root).decisions) {
    const title = r.section ?? ''
    if (!byTitle.has(title)) {
      const sec = { title, rulings: [] }
      byTitle.set(title, sec)
      sections.push(sec)
    }
    byTitle.get(title).rulings.push({
      date:    r.date ?? '',
      id:      r.id ?? null,
      claim:   firstSentence(r.title ?? '', 200),
      section: title,
    })
  }
...
export function renderHtml(model) {
  const parts = [
    ...
    '<!-- generated by: fli ws:atlas --as=report -->',
...
function masthead(model) {
  const counters = [
    ...
    [model.decisions?.count, 'rulings'],
    [model.commands?.total, 'commands'],
  ].filter(([n]) => n)

  return `<header class="masthead">
  <div class="kicker">${esc(model.root)} · generated by fli ws:atlas --as=report</div>
  <h1>Repo map</h1>
...
function registerSection(model) {
  if (!model.issues && !model.decisions && !model.ideas) return ''

  // The three are one section rather than three, because the question they
  // answer together — what should I work on — is answered by reading ACROSS
  // them: `ISSUES.md` alone is how a defect gets fixed that a ruling already
  // retired. Only the open rows are listed. The other two are counted and
  // pointed at, because this page is a printout somebody diffs and 208 rulings
  // in it would drown the eight lines that moved.
  const heads = [
    model.issues    && ['open', model.issues.open, model.issues.file],
    model.decisions && ['rulings', model.decisions.count, model.decisions.file],
    model.ideas     && ['ideas', model.ideas.count, model.ideas.file],
  ].filter(Boolean)

  const lead = `<div class="tallies">${heads.map(([name, n, file]) =>
    `<span class="tally" data-row><b>${esc(name)}</b>${n} <span class="sub">${esc(file)}</span></span>`).join('')}</div>`
  ...
  if (model.issues) {
    ...
    body += `<h3 class="sev">What is wrong <span class="count">${model.issues.open}</span></h3>` +
      `<p class="note">The claim only — each row's evidence, dates and links stay in <code>${esc(model.issues.file)}</code>. ` +
      `A <code>stale?</code> status is a lead, not a fact. ${model.issues.closed} closed rows are not listed.</p>` +
      `<div class="tallies">${spread}</div>` + blocks
  }

  // A section of the settled register runs newest first, so the first ruling
  // under one is the current thinking and is the only one worth a line here.
  if (model.decisions) {
    const rows = model.decisions.sections.map(sec => `<tr data-row>
      <td>${esc(sec.title)}</td>
      <td class="m">${sec.rulings.length}</td>
      <td class="m sub">${esc(sec.rulings[0].id ?? sec.rulings[0].date)}</td>
      <td>${esc(sec.rulings[0].claim)}</td>
    </tr>`).join('')

    body += `<h3 class="sev">What is settled <span class="count">${model.decisions.count}</span></h3>` +
      `<p class="note">By section, with the newest ruling in each — a settled question read as open is the expensive direction, so this is here to be checked against before relitigating. The rest are in <code>${esc(model.decisions.file)}</code>.</p>` +
      table(['Section', 'Rulings', 'Newest', 'Its claim'], rows)
  }

  if (model.ideas) {
    const waves = model.ideas.waves.map(w =>
      `<span class="tally" data-row><b>${esc(w.title)}</b>${w.rows.length}</span>`).join('')

    body += `<h3 class="sev">What is not started <span class="count">${model.ideas.count}</span></h3>` +
      `<p class="note">Ranked rows in <code>${esc(model.ideas.file)}</code>, which declares itself derived — a paper wins where the two disagree. Nothing here is built unless its own Status says so.</p>` +
      `<div class="tallies">${waves}</div>`
  }

  return section('register', 'The three registers',
    heads.map(([name, n]) => `${n} ${name}`).join(' · '),
    `<p class="note">What is wrong, what is settled, what is not started. Read across them.</p>` + lead + body)
}
...
//
// Thirty-odd rows of knowledge paid for one defect at a time, and until this
// the only readings of it were per-package (the atlas plate) and per-diff
// (`fli proves`). The cross-cutting read — the whole table, with each target
// graded — existed nowhere, which is the same gap `invariants.snapshot.md`
// exists to close one document up.

function proofSection(model) {
  if (!model.proofs?.length) return ''

  const targets = model.proofs.flatMap(p => p.targets)
  const missing = targets.filter(t => t.kind === 'unknown').length

  const rows = model.proofs.map(p => {
    const chips = p.targets.map(t =>
      `<span class="tally${t.kind === 'unknown' ? ' bad' : ''}"><b>${esc(t.where)}</b>${esc(t.name)}${
        t.kind === 'unknown' ? ' — not found' : ''}</span>`).join('')
...
// The root markdown FILES, which is a different subject from the three
// registers above: this asks what KIND of statement each file may hold, and
// `PHILOSOPHY.md` §VII is the rule it quotes.
function rootDocSection(model) {
  ...
  return section('root-docs', 'Which file answers which question', 'each one holds a single kind of statement',
...
.tally.bad{background:var(--stop-bg);border-color:var(--stop);color:var(--stop)}
