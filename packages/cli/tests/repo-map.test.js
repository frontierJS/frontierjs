...
//
// The engine behind `fli ws:atlas --as=report`, and the one reader both
// presentations share. What is worth testing is not the page: it is the
// readers, because each one parses a file that was written for a human and may
// be reshaped by one. A reader that silently returns nothing prints a page
// missing a section, which looks like a workspace that does not have one.
//
...
describe('proofs', () => {
  ...
  test('reads the drive-per-change table out of the root CLAUDE.md', () => {
    ...
    expect(proofs[0].changed).toBe('mesa compiler/runtime')
    expect(proofs[0].run).toBe('example: verify and verify:public')
  })

  // The half the markdown cannot show: a row naming a drive that is not there
  // reads exactly like a row that is right, so the page grades each target the
  // way `fli check`'s `proof-target` does and says which ones answer.
  test('each target is resolved, and one that answers to nothing says so', () => {
    const dir = tree('proofs-resolved', {
      'package.json': pkg({ name: 'ws' }),
      // The control is on the same row: `verify` is declared and `verify:gone`
      ...
      'CLAUDE.md': [
        ...
        '| Changed | Run |',
        '| --- | --- |',
        '| the compiler | `example`: `verify` **and** `verify:gone` |',
        '',
        'Prose after the table, which ends it.',
      ].join('\n'),
    })
    ...
    expect(proofs[0].targets.map(t => [t.name, t.kind]))
      .toEqual([['verify', 'row'], ['verify:gone', 'unknown']])

    const html = renderHtml(collect({ root: dir }))
    expect(html).toContain('1 not found')
    expect(html).toContain('verify:gone')
  })
...
describe('the page', () => {
  ...
  test('names its own generator where the snapshot walker reads it', () => {
    ...
    expect(html.split('\n').slice(0, 2)).toEqual(['<!doctype html>', '<!-- generated by: fli ws:atlas --as=report -->'])
    expect(html.indexOf('generated by: fli ws:atlas --as=report')).toBeLessThan(4096)
  })
...
// ─── the three registers ──────────────────────────────────────────────────────
//
// The rulings reader is the one that was measured wrong: this module carried a
// regex of its own that matched only the legacy bold-lead form, every ruling
// migrated to a `###` heading, and the count went to zero — so the page showed
// one register of three and said nothing. It reads through `core/registers.js`
// now, the one owner, and the guard is the migrated form.
...
describe('the three registers', () => {
  ...
  test('a ruling in the migrated heading form is read, and grouped by section', () => {
    const { decisions } = collect({ root: withRegisters('reg-read') })

    expect(decisions.count).toBe(3)
    expect(decisions.sections.map(s => [s.title, s.rulings.length]))
      .toEqual([['Access control', 2], ['Naming', 1]])
  })

  // A section runs newest first, so the first ruling under one is the current
  // thinking — which is the only one the page has room to quote.
  test('the newest ruling in a section is the one at the top of it', () => {
    const { decisions } = collect({ root: withRegisters('reg-newest') })
