...
// The pause guard and the page it serves. Both come from `core/pause.js`, which
// is also what `deploy:pause` writes against and what `deploy:status` greps for
// — a vhost that carried its own copy of the guard would be a second answer to
// where the file lives.
const { nginxGuard, DEFAULT_PAGE, pagePath, fliDir, vhostPath } =
