---
title: ports:claim
description: Claim a port session for the current project and inject FLI_PORT_* into the environment
alias: claim
examples:
  - fli claim
  - fli claim --env test
...
// Same preflight `utils:dev` runs, and for the same reason: an app with an
// empty database boots clean and shows a blank screen, so the state is said out
// loud rather than discovered. One owner, two entry points, because a person
// claiming ports is about to start the servers this warns about.
const { warnIfDatabaseEmpty } = await import(resolve(global.fliRoot, 'core/db-preflight.js'))
if (!flag['no-check']) {
  try { warnIfDatabaseEmpty(context) }
  catch { /* a courtesy must never stop a dev server */ }
}
