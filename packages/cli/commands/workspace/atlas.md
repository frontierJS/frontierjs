...
description: Write the field atlas — one plate per package, each opening into a dossier that crosses the registers
alias: ws:atlas
examples:
  - fli ws:atlas
  - fli ws:atlas --open
...
flags:
  check:
    char: c
    type: boolean
    description: Exit 1 if the committed atlas no longer matches the workspace
    defaultValue: false
  stdout:
    type: boolean
  ...
  out:
    char: o
    type: string
    description: Output path (default is repo-atlas.snapshot.html at the workspace root)
    defaultValue: ''
---
...
const { collect }               = await import(resolve(global.fliRoot, 'core/repo-map.js'))
...
// Before anything is read: `--live` shells out to git and to the registry, and
// spending that on a run whose answer is *these two flags do not go together*
// is a slow way to say no.
...
const body = renderAtlas(model, live)

if (flag.stdout) { echo(body); return }

const outPath = flag.out
  ? resolve(flag.out)
  : resolve(wsRoot, flag.live ? 'repo-atlas.live.html' : 'repo-atlas.snapshot.html')
const shown   = outPath.replace(wsRoot + '/', '')

if (flag.check) {
  if (!existsSync(outPath)) {
    log.error(`No atlas at ${outPath} — run \`fli ws:atlas\` and commit it.`)
    process.exitCode = 1
    return
  }
  ...
  echo('')
  echo('  The workspace changed. Run `fli ws:atlas` and review the diff before committing.')
  process.exitCode = 1
  return
}
...
// The atlas counts the snapshots in the workspace and, once written, is one —
// so the first generation is a plate short of itself and `--check` would fail
// on a file nobody had touched. Writing twice on creation lands on the fixed
// point; every run after this one is already there. The live page is not
// counted and needs none of this.
if (!existed && !flag.live) writeFileSync(outPath, renderAtlas(collect({ root: wsRoot })), 'utf8')
...
## Its relationship to `ws:map`

Same model, different question. `fli ws:map` answers *what do I run and where*
in one scannable page; the atlas answers *what is in here and what does it
touch*. A page that answered both would answer neither.
