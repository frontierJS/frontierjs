...
// ─── admin/_routes.js ─────────────────────────────────────────────────────────
//
// Model name → where that model's list lives. A detail page links each of its
// child collections into that list filtered by the row, and the route is the
// one thing the page cannot derive: `resource.children()` answers the child's
// model and the foreign key pointing back, and only this command knows the
// folder the child landed in.
//
// One module rather than a copy per page: thirty-two inlined maps is the same
// written-twice failure `core/crud-templates.js` exists to end, and a
// single-model regeneration would leave thirty-one of them stale.

function makeRoutes(targets) {
  const rows = targets.map(t => `  ${JSON.stringify(t.name)}: '/admin/${t.key}/',`).join('\n')
  return `// web/src/routes/admin/_routes.js — generated by \`fli admin:generate\`.
//
// Model name → its list route. Read by every detail page to link a child
// collection at the list that holds it, filtered by the row you are looking at.
// A model this admin does not cover is absent, and a link is not offered for it
// rather than pointing at a route nothing serves.
...
// `childRoutes` is model name → base path for every model THIS RUN writes. The
// detail page links each child collection into its own list filtered by the
// row, and the route is the one thing it cannot derive: a child knows its model
// and its foreign key, and only the loop below knows the folder that model
// landed in. Built from the same `m.key` the folder is named for, so a link
// cannot point at a directory this command did not write — and a child outside
// the target set is simply absent, which is what keeps `--model One` safe.
const adminPages = (m, paths, res) => {
  ...
  return {
    ...
    detail: editPage({
      ...shared,
      childRoutesImport: `import { childRoutes } from '${paths.routes}'`,
...
const pathsAt = (depth) => ({
  ...
  routes:    depth === 2 ? './_routes.js'    : '../'.repeat(depth - 2) + '_routes.js',
  session:   ownSession
...
write(resolve(adminDir, '_routes.js'),   makeRoutes(targets), 'admin/_routes.js')
