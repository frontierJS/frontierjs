---
title: ksite:setup
description: Walk through first-time setup for a freshly cloned site (rm boilerplate, set domain, init stage branch)
examples:
  - fli ksite:setup
  - fli ksite:setup --dry
  - fli ksite:setup --force
  - fli ksite:setup --skip git
flags:
  force:
    type: boolean
...
Setup is a one-shot operation: it sets `config_ranSetup: true` in
...
if (!existsSync(systemFile)) {
  ...
  log.info('site:setup only works on ksite-cloned sites with a content/settings/ folder')
  return
}
...
if (hasRanSetup && !flag.force) {
  log.info('Site setup has already been run (config_ranSetup: true in system.md)')
