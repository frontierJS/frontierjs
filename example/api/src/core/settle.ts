// api/src/core/settle.ts — an order has been paid for.
//
 ...
 * order whose basket has since been swept — see `api/src/inventory.ts`.
 *
...
export async function refundOrder(orderId: number): Promise<SettledOrder> {
  ...
  // at every call site (`api/src/inventory.ts`).
